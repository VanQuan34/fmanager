...
export class MoWbLoginComponent implements OnInit {
  ...
  async login() {
    ...
    const response = await this.authService.login(username, password ).then((res)=>{
      if (res) {
        console.log('oke');
        ...
        this._router.navigate([redirectUrl || '/dashboard']);
  ...
  validate(){
    if(this.userNameInput.nativeElement.value.trim() === ''){
  ...
  private async authenticate(redirectLoginError?: boolean) {
    ...
    timer(1000).subscribe(() => {
      ...
      // const newHref = `${loc.protocol}//${loc.hostname}`;
